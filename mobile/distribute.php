...
elseif($action == 'userrank'){
	...
	if(!empty($up_uid_all_two))
	{
		foreach($up_uid_all_two as $key =>$value)
		{
			$sql = "SELECT  user_id  FROM " . $ecs->table('users') . " WHERE parent_id=$value[user_id]";
			$up_uid_all_tree=$GLOBALS['db']->getAll($sql);
			if($up_uid_all_tree[0][user_id]!=null)
			{
				$user_array2[]=$up_uid_all_tree[0][user_id];
			}
			if(!empty($up_uid_all_tree))
			{
				foreach($up_uid_all_tree as $key =>$value )
				{
					$sql = "SELECT  user_id  FROM " . $ecs->table('users') . " WHERE parent_id='$value[user_id]'";
					$up_uid_all_four=$GLOBALS['db']->getAll($sql);
					if($up_uid_all_four[0][user_id]!=null)
					{
						$user_array3[]=$up_uid_all_four[0][user_id];
					}
					if(!empty($up_uid_all_four))
					{
						foreach($up_uid_all_four as $key =>$value)
						{
							$sql = "SELECT  user_id  FROM " . $ecs->table('users') . " WHERE parent_id=$value[user_id]";
							$up_uid_all_five=$GLOBALS['db']->getAll($sql);
							if($up_uid_all_five[0][user_id]!=null)
							{
								$user_array4[]=$up_uid_all_five[0][user_id];
							}
							if(!empty($up_uid_all_five)){
								foreach($up_uid_all_five as $key =>$value)
								{					
	...
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
	$num =  count($affiliate['item']);//获取分销级别
	$menuarr=array();
	for ($i = 1; $i<=$num; $i++)
	{
		$smarty->assign('count'.$i,        $res[$i]);
	...
	/* 获取客户订单总额 */
	foreach($user_array1 as $value)
	{
		$sql = "SELECT  sum(order_amount ) as sum_order  FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE user_id=$value[user_id] and order_status = '1' and shipping_status = '2'";
		$count = $GLOBALS['db']->getOne($sql);
		$sql1="select user_name from" . $GLOBALS['ecs']->table('users') ." WHERE user_id=$value[user_id]";
		$user_name=$GLOBALS['db']->getOne($sql1);
		$user_list=array();
		if ($count!='')
		{
			$user_list[id]=$value[user_id];
			$user_list[user_name]=$user_name;
			$user_list[money]=$count;
			$user_list[status]="一级会员";	
			$user_list1[]=$user_list;
		}
	}
	
	/* 	二级分销列表 */
	foreach($user_array2 as $value)
	{
		$sql = "SELECT  sum(order_amount ) as sum_order  FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE user_id=$value and order_status = '1' and shipping_status = '2'";
		$count = $GLOBALS['db']->getOne($sql);
		$sql1="select user_name from" . $GLOBALS['ecs']->table('users') ." WHERE user_id=$value";
		$user_name=$GLOBALS['db']->getOne($sql1);
		$user_list=array();
		if ($count!='')
		{
			...
			$user_list[user_name]=$user_name;
			$user_list[money]=$count;
			$user_list[status]="二级会员";	
			$user_list2[]=$user_list;
		}
		
	}
	/* 三级分销列表 */
		foreach($user_array3 as $value)
	{
		$sql = "SELECT  sum(order_amount ) as sum_order  FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE user_id=$value and order_status = '1' and shipping_status = '2'";
		$count = $GLOBALS['db']->getOne($sql);
		$sql1="select user_name from" . $GLOBALS['ecs']->table('users') ." WHERE user_id=$value";
		...
		if ($count!='')
		{
			...
			$user_list[user_name]=$user_name;
			$user_list[money]=$count;
			$user_list[status]="三级会员";	
			$user_list3[]=$user_list;
		}
		
	}
	/* 四级会员列表 */
		foreach($user_array4 as $value)
	{
		$sql = "SELECT  sum(order_amount ) as sum_order  FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE user_id=$value and order_status = '1' and shipping_status = '2'";
		$count = $GLOBALS['db']->getOne($sql);
		$sql1="select user_name from" . $GLOBALS['ecs']->table('users') ." WHERE user_id=$value";
		...
		if ($count!='')
		{
			...
			$user_list[user_name]=$user_name;
			$user_list[money]=$count;
			$user_list[status]="四级会员";	
			$user_list4[]=$user_list;
		}
		
	}
	/* 五级会员列表 */
	foreach($user_array5 as $value)
	{
		$sql = "SELECT  sum(order_amount ) as sum_order  FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE user_id=$value and order_status = '1' and shipping_status = '2'";
		$count = $GLOBALS['db']->getOne($sql);
		$sql1="select user_name from" . $GLOBALS['ecs']->table('users') ." WHERE user_id=$value";
		...
		if ($count!='')
		{
			...
			$user_list[user_name]=$user_name;
			$user_list[money]=$count;
			$user_list[status]="五级会员";	
			$user_list5[]=$user_list;
		}
	...
	if(!empty($user_list1)){
		...
		if(!empty($user_list2)){
			...
			if(!empty($user_list3)){
				$user_list=array_merge($user_list1,$user_list2,$user_list3);
				if(!empty($user_list4)){
					$user_list=array_merge($user_list1,$user_list2,$user_list3,$user_list4);
					if(!empty($user_list5)){
						$user_list=array_merge($user_list1,$user_list2,$user_list3,$user_list4,$user_list5);
					}
				}
			}
		}
	...
	array_multisort($money, SORT_DESC, $data, SORT_DESC, $user_list);
...
elseif ($action == 'account_raply')
{
    $user_id = $_SESSION['user_id'];
    if (!$_POST['bind']){
    	$password_account_raplay = $db -> getOne("select password_account_replay from ".$ecs->table('users')." where user_id = $user_id");
    	if (empty($password_account_raplay)){
